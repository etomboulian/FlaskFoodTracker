...
def index():
    ...
    if request.method == 'POST':
        date = request.form['date'] # assume always in YYYY-MM-DD format
    ...
    for i in results:
        ...
        single_date['stats'] = stats
        nice_results.append(single_date)
    


    return render_template('home.html', results=nice_results)

@app.route('/view/<string:date>', methods=['GET', 'POST'])
def view(date):
    db = get_db()
    ...
    if request.method == 'POST':
        ...
        db.execute('INSERT INTO food_date (food_id, log_date_id) values (?,?)', [selection, date_result['id']])
        db.commit()
    ...
    return render_template('day.html', date=date, pretty_date=pretty_date, food_results=food_results, log_results=log_results, totals=totals)
